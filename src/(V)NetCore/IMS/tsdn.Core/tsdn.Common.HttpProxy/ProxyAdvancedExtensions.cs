        ...
        public static async Task CopyProxyHttpResponse(this HttpContext context, HttpResponseMessage responseMessage)
        {
            ...
            foreach (var header in responseMessage.Content.Headers)
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        string headeValue = string.Join("", header.Value);
                        var arr = headeValue.Split(';');
                        for (int i = 0; i < arr.Length; i++)
                        {
                            string detail = arr[i];
                            if (detail.StartsWith("filename=", StringComparison.CurrentCultureIgnoreCase))
                            {
                                arr[i] = "filename=" + DateTime.Now.ToString("yyyyMMddHHmmss") + System.IO.Path.GetExtension(detail);
                                hasHandle = true;
                            }
                        }
                        if (hasHandle)
                        {
                            response.Headers[header.Key] = new string[] { string.Join(";", arr) };
                        }
                    }
                    if (!hasHandle)
                    {
