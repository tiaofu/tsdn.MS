...
namespace tsdn.Utility.Excel
{
    ...
    public abstract class ExcelImport : ITransientDependency
    {
        ...
        /// <summary>
        /// 是否需要用户信息，默认为false,需要用户信息是请重写该属性
        /// </summary>
        public virtual bool DowloadNeedUserInfo
        {
            get
            {
                return false;
        ...
        public async virtual Task GetExportTemplate(string FilePath, Stream s)
        {
        ...
        /// <returns>模版MemoryStream</returns>
        public async virtual Task GetExportTemplate(string FilePath, Stream s, UserInfo userInfo)
        {
            using (FileStream fs = File.OpenRead(FilePath))
            {
                await fs.CopyToAsync(s);
        ...
        public virtual Task<ImportResult> ImportTemplate(Stream ins, string fileName, UserInfo userInfo)
        {
            if (DictFields == null)
            {
            ...
            return Task.Factory.StartNew(() =>
            {
                ...
                if (DowloadNeedUserInfo)
                {
                    //4.获取校验所需额外参数
                    extraInfo = GetExtraInfo(columns, dt, userInfo);
                }
                else
                {
                    //4.获取校验所需额外参数
        ...
        /// 例如导入excel中含有下拉框 导入时需要判断选项值是否还存在，可以通过该方法查询选项值
        /// </remarks>
        public virtual Dictionary<string, object> GetExtraInfo(List<string> listColumn, DataTable dt, UserInfo userInfo)
        {
            return null;
        }
