	...
	public static class XmlCommandManager
	{
        ...
        public static void UnSafeInit()
        {
            ...
            if (false == string.IsNullOrEmpty(folder))
            {
                ...
                {
                    folders[i] = System.IO.Path.Combine(AppContext.BaseDirectory, folders[i]);
                }
                LoadCommnads(folders);
            }
        }
        ...
        /// </summary>
        private static readonly bool IsAspnetEnvironment = false;// (HttpRuntime.AppDomainAppId != null);
